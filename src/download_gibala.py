"""Download the Gibala et al. prostate T2/zone segmentation dataset from Zenodo."""
...
DEFAULT_RECORD_ID = 7676958
DEFAULT_FOLDER = "prostate_dataset"
REQUIRED_PATHS = ("PSA_PIRADS.csv",)
SOURCE_NAME = "zenodo"
DATASET_NAME = "gibala_prostate_zones"
...
def download_dataset(
    ...
    with tempfile.TemporaryDirectory(dir=tmp_root) as tmp_dir:
        ...
        if len(archives) != 1:
            raise RuntimeError(
                f"Expected exactly one downloaded zip for record {record_id}, "
                f"found {len(archives)}."
            )

        with zipfile.ZipFile(archives[0]) as zf:
            zf.extractall(record_root)
